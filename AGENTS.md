...
6. **The model list is server-driven, not hardcoded.** kimi-cli keeps no static model list at all: `refresh_managed_models` (`research/kimi-cli/src/kimi_cli/auth/platforms.py`) treats `GET /coding/v1/models` as the full truth — adding new models, updating metadata, and removing gone ones. We mirror that: the `config` hook refreshes the stored token when it is expiring (Kimi access tokens live ~15 min, so a cold start almost always needs it — without the refresh, startup discovery 401s and silently degrades to the fallback) and injects every discovered model into opencode's config at startup (union semantics — user-written entries are never deleted, which is the one deliberate deviation from kimi-cli's remove-gone behavior, because opencode config is user-owned); `provider.models` materializes discovered models missing from the runtime map; the chat hooks gate on the discovered id set once warm (fallback-id models the server omits drop out of the gate, so their requests fail loud server-side instead of carrying Kimi-specific fields); `rememberDiscovery` overwrites even on an empty list because a successful response is authoritative. The static `FALLBACK_MODELS` in `src/constants.ts` exists ONLY for the cold-start window (offline start, not logged in, refresh-and-retry failed) — do not treat it as the model list, and do not add new models there unless they also need to work with zero connectivity. The wire `model` rewrite in `loader.fetch` sends whatever id the server returned; the legacy `MODEL_ID → discovered slug` fallback applies ONLY when discovery returned exactly one model — with several models, guessing `models[0]` could silently run the request on a different model than the user selected, so the id stays on the wire and the server answers with a visible entitlement error. `MAX_REASONING_MODEL_IDS` is a separate server-side capability gate (which models accept `reasoning_effort: "max"`), not a model list — unknown/new models clamp `max`→`high` until verified, because an unsupported effort value fails the request upstream.
